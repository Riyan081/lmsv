...
export default function AdminLeaveClient({ leaves }: { leaves: LeaveItem[] }) {
  ...
  }, [leaves, statusTab, deptFilter, roleFilter, search]);
  ...
  return (
    <div className="space-y-5">
      {/* Status Tabs */}
      <div className="flex items-center gap-2 border-b pb-4 overflow-x-auto" style={{ borderColor: "var(--color-border)" }}>
        {[
          { key: "all", label: "All Leaves", count: counts.all },
          { key: "pending", label: "Pending", count: counts.pending, badgeColor: "bg-amber-500/20 text-amber-400" },
          { key: "approved", label: "Approved", count: counts.approved, badgeColor: "bg-emerald-500/20 text-emerald-400" },
          { key: "rejected", label: "Rejected", count: counts.rejected, badgeColor: "bg-red-500/20 text-red-400" },
        ].map((tab) => {
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          <select value={deptFilter} onChange={(e) => setDeptFilter(e.target.value)} className={inputCls} style={inputStyle}>
          ...
          <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className={inputCls} style={inputStyle}>
          ...
          {(deptFilter || roleFilter || search) && (
            <button
              onClick={() => { setDeptFilter(""); setRoleFilter(""); setSearch(""); }}
      ...
      {filteredLeaves.length === 0 ? (
        <div className="rounded-2xl border p-12 text-center" style={{ background: "var(--color-bg-card)", borderColor: "var(--color-border)" }}>
          ...
          <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No leave applications match your filters.</p>
        </div>
      ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      ...
                      <span className="text-xs uppercase px-2 py-0.5 rounded-md bg-white/5 font-medium" style={{ color: "var(--color-text-muted)" }}>
                        {leave.user?.role}
                      </span>
                      {leave.user?.enrollmentNo && (
                        <span className="text-xs px-2 py-0.5 rounded-md bg-white/5" style={{ color: "var(--color-text-muted)" }}>
                          {leave.user.enrollmentNo}
                        </span>
                      )}
                    ...
                    {/* Metadata Footer */}
                    <div className="flex flex-wrap items-center gap-2.5 mt-3 text-xs" style={{ color: "var(--color-text-muted)" }}>
                      <span className="capitalize font-medium text-purple-400 px-2 py-0.5 rounded bg-purple-500/10">
                        {leave.type} leave
                      </span>
                      <span>·</span>
                      <span>
                        📅 {startDate.toLocaleDateString("en-IN", { day: "2-digit", month: "short" })} — {endDate.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })} ({days} day{days !== 1 ? "s" : ""})
                      </span>
                      <span>·</span>
